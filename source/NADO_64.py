...
def Complex1_s(a) :
    m_real = torch.Tensor(a).uniform_(-0.3,0.02).type(torch.complex128)
    m_img = torch.Tensor(a).normal_(0,0.05).type(torch.complex128)
    m = m_real + (1.j)*m_img
    return m

def Complex2_sd(a,b) :
    m_real = torch.Tensor(a,b).uniform_(-0.3,0.02).type(torch.complex128)
    m_img = torch.Tensor(a,b).normal_(0,0.1).type(torch.complex128)
    m = m_real + (1.j)*m_img
    return m

def Complex1_h(a) :
    m_real = torch.Tensor(a).uniform_(-0.01,0.01).type(torch.complex128)
    m_img = torch.Tensor(a).normal_(0,0.01).type(torch.complex128)
    m = m_real + (1.j)*m_img
    return m

def Complex2_sh(a,b) :
    m_real = torch.Tensor(a,b).uniform_(-0.01,0.01).type(torch.complex128)
    m_img = torch.Tensor(a,b).normal_(0,0.01).type(torch.complex128)
    m = m_real + (1.j)*m_img
    return m
...
class NADO(nn.Module):
    def __init__(
        ...
        print(f'Nh:{self.Nh:d}; Na:{self.Na:d}')
        print(f'number of parameters:{self.nparameters:d}')
        print(f'h:{num_Nh:d}; a:{num_Na:d} ; sd:{2*2*self.Nd*self.Ns:d}',flush=True)

        self.bs = nn.Parameter(Complex1_s(self.Ns))
        self.bd = nn.Parameter(torch.Tensor(self.Nd).type(torch.float64).uniform_(-1.2,-0.6))
        ...
        self.w_da = nn.Parameter(torch.Tensor(self.Nd, self.Na).type(torch.float64).uniform_(-0.01,0.01))
        self.ba = nn.Parameter(torch.Tensor(self.Na).type(torch.float64).uniform_(-0.01,0.01))
    ...
    def States(self, x) :
        ...
        y,x1,x2 = y.type(torch.complex128), x1.type(torch.complex128), x2.type(torch.complex128)
        r1 = torch.exp(y.matmul(2*self.bd.type(torch.complex128)) + 
        ...
        x1_c, x2_c = x2, x1
        r1_c = torch.exp(y_c.matmul(2*self.bd.type(torch.complex128)) + 
    ...
    def Gradients_t(self,x):
        n = x.shape[0]
        gradients = torch.zeros((n,self.nparameters),dtype=torch.complex128,device=x.device)
        n_part = 100
    ...
    def State(self, x):    
        ...
        r1 = torch.exp(y.matmul((self.bd + self.bd).type(torch.complex128)) + 
                    x1.matmul(self.bs) + torch.conj(x2.matmul(self.bs)) + 
        ...
        x2_c = x1
        x1_c = x2
        r1_c = torch.exp(y_c.matmul((self.bd+self.bd).type(torch.complex128)) + 
                    x1_c.matmul(self.bs) + torch.conj(x2_c.matmul(self.bs)) + 
    ...
    def f3(self, x1, x2, y):
        a = (2*self.ba.type(torch.complex128) + 
            x1.matmul(self.w_sa) + torch.conj(x2.matmul(self.w_sa)) + 
            y.matmul((self.w_da + self.w_da).type(torch.complex128)))
        return a
    ...
    def rho_0(self) :
        ...
        state0 = torch.zeros((self.table0.shape[0],self.nstate),dtype=torch.int8,device=self.device)
    ...
    def rho(self) :
        m = 2**self.nstate
        rho = torch.zeros(m,dtype=torch.complex128)
