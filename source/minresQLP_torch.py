...
def MinresQLP_torch_64(A, b, rtol, maxit, M=None, shift=None, maxxnorm=None,
...
#vector
    ...
    tau = torch.zeros(1,dtype=torch.float64,device=device)
    taul = 0
    phi = beta1
    betan = beta1
    gmin = 0
    ...
    veplnl = 0
    veplnl2 = 0
    ul3 = torch.zeros(1,dtype=torch.float64,device=device)
    ul2 = torch.zeros(1,dtype=torch.float64,device=device)
    ul = 0
    u = 0
    rnorm = betan
    xnorm = 0
    xl2norm = torch.zeros(1,dtype=torch.float64,device=device)
    Axnorm = 0
    Anorm = 0
    Acond = 1
    relres = rnorm / (beta1 + 1e-50)
#vector
    x = torch.zeros((n,1),dtype=torch.float64,device=device)
    w = torch.zeros((n,1),dtype=torch.float64,device=device)
    wl = torch.zeros((n,1),dtype=torch.float64,device=device)
    if rnormvec:
...
#vector
        ...
        if abs(gama) > torch.finfo(torch.double).tiny and xnorm_tmp < maxxnorm:
        ...
        #Minres-QLP
            ...
            QLPiter += 1
            if QLPiter == 1:
                xl2 = torch.zeros((n,1),dtype=torch.float64,device=device)
...
#vector                    
        ...
        epsx = Anorm * xnorm * torch.finfo(torch.float64).eps
...
#vector
    ...
    relres = rnorm/(Anorm*xnorm + beta1)
    relAres = 0
    if rnorm > torch.finfo(torch.double).tiny:
    ...
    if rnormvec:
        ...
        Aresvec.append(Arnorm)
        return (x.flatten(),flag,iters,Miter,QLPiter,relres,relAres,Anorm,Acond,
                xnorm,Axnorm,resvec,Aresvec)
    
    return (x.flatten(),flag,iters,Miter,QLPiter,relres,relAres,Anorm,Acond,xnorm,Axnorm)
...
##################    example1    ####################
    n=100
    e = torch.ones((n,1),dtype=torch.float64,device=device)
    data = torch.c_[-2*e,4*e,-2*e]
